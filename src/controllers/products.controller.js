...
export async function getAllProducts(req, res) {
    try {
        const { category, brand, min_price, max_price, min_rating, page = '1', limit = '50' } = req.query;
        let query = `
      SELECT 
        p.*,
 ...
 */
export async function getProductByAsin(req, res) {
    try {
    ...
    `;
        const startTime = Date.now();
        const [products] = await pool.query(query, [asin]);
        const responseTime = ((Date.now() - startTime) / 1000).toFixed(2);
...
export async function updateProductPrice(req, res) {
    try {
        ...
        await pool.query('UPDATE amazon_products SET price = ?, updated_at = NOW() WHERE asin = ?', [parseFloat(price), asin]);
        await pool.query('UPDATE amazon_buybox_offers SET price = ?, updated_at = NOW() WHERE asin = ?', [parseFloat(price), asin]);
        res.json({
            success: true,
